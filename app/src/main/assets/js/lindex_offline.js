...
//获取搜索框数据和筛选条件，传递搜索参数
function getInput(){
    var input=$et('search').value.replace(/\s+$/g,'');
...
function combinationUrl(input_value,type){
    ...
    var url_template=`${value_},${format_},${appendArea_}`
...
function AlinkService(){
    ...
    if(url!='kull'){
        //这里写入网站api；
        var final_url=''
        if (url.includes('hz,kull')){
            final_url='hkoffline/search_hz_'+url;
        }
        else{
            final_url='hkoffline/search_pin_'+url;
        }
        
        if(final_url.includes("hz,kull")){
            returnSkeleton(1,'');
        }else{
            returnSkeleton(2,'');
        }
        setTimeout(()=>{
            app.OfAjax(final_url)
        },200)
...
function ResultCard(chract){
    ...
    return chard_failure;
        ...
        var ps=chract.ps
        var detail_link="hkoffline://search_/?s="+hz;
...
function ResultPinCard(pinract){
    ...
    //判断json中的pin值是否为空
    if (pins==''|pins==null|pins==undefined){
                        ...
                        <div class="accordion-collapse mt-0 collapse" id="${tag}" style="">
                            <div class="accordion-body p-2">
                                ...
                                <div class="col offset-9 p-0">
                                    <a class="acopy p-0" onclick="returnCopys(${tag})"><small>复制文本</small></a>
                                </div>
