...
public class DownloadManager {
    ...
    public void DeleteDownTask(DownloadBean bean) throws IOException {
        File file = new File(bean.save_path);
    ...
    public class DownLoadTask implements Runnable {
        ...
        public void run() {
            ...
            File file = new File(bean.save_path);// 获取下载文件
            if (!file.exists()) {
                // 如果文件不存在
                bean.progress = 0;
                try {
            ...
            try{
                    ...
                    try{
                        ...
                        long total_progress= Objects.requireNonNull(response.body()).contentLength();
                        Log.i( "received from server_inputstream: ",String.valueOf(total_progress));
                        if((int)total_progress!=-1){
                            bean.appSize=total_progress;
                            notifyDownloadStateChanged(bean);
                        }
                        ...
                        if (code >=200) {
                            ...
                            while ((length = is.read(buffer)) != -1) {
                                ...
                                if (bean.download_state == STATE_PAUSED) {
                                    ...
                                    notifyDownloadStateChanged(bean);
                                    return;
                                } else if (bean.download_state == STATE_DELETE) {
                                    // 下载的时候删除直接回调界面，然后直接返回
                                    bean.download_state = STATE_DELETE;
                                    notifyDownloadStateChanged(bean);
                                    outputStream.close();
                                    is.close();
                            ...
                            notifyDownloadStateChanged(bean);
                            if (total_progress == compeleteSize|total_progress!=compeleteSize) {
                                bean.download_state = STATE_DOWNLOADED;
                                //DataBaseUtil.UpdateDownLoadById(bean);
